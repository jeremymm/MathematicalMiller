...
void TestNormalizeVector()
{
  ...
  for(unsigned long i = 0; i < ULONG_MAX; i++)
  {
    ...
    MST_VECTOR2.X(DX_VECTOR2.x);
    MST_VECTOR2.Y(DX_VECTOR2.y);
    // test normalize

    D3DXVECTOR2 DX_VECTOR2_NORMAL;
    D3DXVec2Normalize(&DX_VECTOR2_NORMAL, &DX_VECTOR2);
    MST_VECTOR2.Normalize();

    float xDiff = DX_VECTOR2_NORMAL.x - MST_VECTOR2.X();
    float yDiff = DX_VECTOR2_NORMAL.y - MST_VECTOR2.Y();
    ...
    if((i % 100000) == 0)
    {
      cout << "i: " << i << endl 
        ...
        << " X Diff Avg: " << fxDiffTotal / static_cast<float>(100000) << " Y Diff Total: " << fyDiffTotal / static_cast<float>(100000) << endl;
      fyDiffTotal = 0.0;
      fxDiffTotal = 0.0;
    }
  }
